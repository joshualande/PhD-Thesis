...
Spatial extension is an important characteristic for correctly associating
$\gamma$-ray-emitting sources with their counterparts at other wavelengths
and for obtaining an unbiased model of their spectra.  We present a
new method for quantifying the spatial extension of sources detected by
the Large Area Telescope (LAT), the primary science instrument on the
{\em \fermi Gamma-ray Space Telescope} (\fermi).  We perform a series
of Monte Carlo simulations to validate this tool and calculate the LAT
threshold for detecting the spatial extension of sources.
...
This type of analysis is unwieldy using the standard
...
\subsecref{monte_carlo_validation}, simultaneously fitting the position,
...
\begin{figure}[htbp]
  ...
  PSF (inset). Both were smoothed by a 0\fdg1 2D Gaussian kernel.
...
We estimate the error on the extension of a source by fixing the
...
In \subsecref{bias_wrong_spatial_model}, we perform a dedicated Monte
...
We tested the theoretical distribution for \tsext to evaluate
...
form of \eqnref{ts_ext_distribution} is due to the null hypothesis
...
\eqnref{ts_ext_distribution}, we evaluated the empirical distribution
...
\begin{figure}[htbp]
    ...
    is the cumulative density function of \eqnref{ts_ext_distribution}.}
    \figlabel{ts_ext_mc_1000}
  \end{figure}

\begin{figure}[htbp]
    \includegraphics{chapters/extended_analysis/figures/mc_plots/ts_ext_emin_10000_color.pdf}
    \caption{The same plot as \figref{ts_ext_mc_1000} but fitting in
...
were discarded.  \tabref{ts_ext_num_sims} shows the different spectral
...
is plotted in \figref{ts_ext_mc_1000} and \figref{ts_ext_mc_10000} and
compared to the $\chi^2_1/2$ distribution of \eqnref{ts_ext_distribution}.

Our study shows broad agreement between simulations and
\eqnref{ts_ext_distribution}. To the extent that there is a discrepancy,
...
distribution in \figref{ts_ext_mc_1000} and \figref{ts_ext_mc_10000},
...
four spectral models, \tabref{ts_ext_num_sims} lists $F(\text{GC})$
...
\figref{tsext_plane_plot} shows the cumulative density of \tsext for each
...
100 \gev and a spectral index of 2.66 (see \secref{validate_known}).
...
a semi-major axis of 0\fdg3, a semi-minor axis of 0\fdg19, a position
...
\subsecref{validation_over_plane}, but the simulations were over the
...
\figref{ts_comparison_w44sim}a shows that the significance of W44 in the
simulations is very large ($\ts\sim3500$) for a model with a point-like
source hypothesis.  \figref{ts_comparison_w44sim}b shows that the
...
\figref{ts_comparison_w44sim}c shows that the sources were fit better
...
Finally, \figref{ts_comparison_w44sim}d shows that the sources
...
\figref{bias_w44sim}a and \figref{bias_w44sim}b clearly show that no
...
\seclabel{extension_sensitivity}

We calculated the LAT flux threshold to detect spatial extent. We define
...
We used a simulation setup similar to that described in
\subsecref{monte_carlo_validation}, but instead of point-like sources we
...
with increasing source size and reaches a minimum around 0\fdg5.  The
threshold increases for large extended sources because the source becomes
increasingly diluted by the background.  \figref{index_sensitivity}
...
\mev and 1 \gev.  This is also demonstrated in \figref{four_plots_ic443}
...
(\secref{extended_source_search_method}), we therefore used only photons
...
\figref{all_sensitivity} are the LAT-detected extended sources that will
be discussed in \secref{validate_known} and \secref{new_ext_srcs_section}.
The extension thresholds are tabulated in \tabref{all_sensitivity_table}.
...
Finally, \figref{time_sensitivity} shows the projected detection
...
We considered using the Bayesian information criterion
...
as in \secref{extension_sensitivity} against a background 10 times the
...
\secref{new_ext_srcs_section}).  We simulated these sources using
uniform disk spatial models with extensions varying up to $1\degree$.
\figref{confusion_extended_plot}a shows the distribution of \tsext and
\tsinc and \figref{confusion_extended_plot}c shows the distribution of
...
\figref{confusion_2pts_plot}a shows the same plot but when fitting two
...
Finally, \figref{confusion_2pts_plot}c shows the same plot with each
...
The same four plots are shown in \figref{confusion_extended_plot}b,
\figref{confusion_extended_plot}d, \figref{confusion_2pts_plot}b, and
\figref{confusion_2pts_plot}d but this time when analyzing a source of
...
\gev and 100 \gev (see \secref{new_ext_srcs_section}).
...
data of spatially extended sources.  \subsecref{monte_carlo_validation}
...
\begin{figure}[htbp]
  ...
  to follow a $\chi^2_1/2$ distribution (\eqnref{ts_ext_distribution},
...
in \figref{agn_ts_ext}.  The \tsext distribution for the AGN shows
...
(see \secref{systematic_errors_on_extension}).  The discrepancy could
also in a few cases be due to confusion with a nearby undetected source.
We note that the Monte Carlo study of \subsecref{monte_carlo_validation}
...
\secref{systematic_errors_on_extension}). Finding that the AGN we test
