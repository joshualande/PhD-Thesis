...
In that work, we analyzed the $\gamma$-ray emitting \acp{PWN} \hessj{1825}
...
\hessj{1825} and \hessj{1837}).  We therefore suspect that this search
...
we included \hessj{1023}. Although this source is classified as a massive
...
as \PWNClass-type or \PWNcClass-type when the emission has a
...
should be due to a pulsar-wind nebula.  We label a source as \PWNClass-type
...
for each \PWNClass-type source in \subsecref{pwn_pwnc_sources}.  We include
...
In total, we detect fourteen sources which we classify as \PWNClass-type or
\PWNcClass-type. Five of these \ac{PWN} and \ac{PWN} candidates are first
...
\hessj{1825} is classified as \PWNClass-type because of the
...
In addition, we detect eleven \PWNcClass-type sources.  Four of these sources
are first reported in this work: \hessj{1119}, \hessj{1303}, \hessj{1420},
and \hessj{1841}.  These sources are all powered by pulsars energetic
...
In \chapref{offpeak}, \psrj{1119} has $\ts=61.3$ and is classified as
...
We detect six \OtherClass-type sources.

Two of these sources (\hessj{1634} and \hessj{1804}) have a hard spectrum
which connects spectrally to the \ac{VHE} emission, but are not classified
as \ac{PWN} based upon muliwavelenth considerations.  For \hessj{1634},
is not a \ac{PWN} candidate because there are no pulsar counterparts able
to power it.  Finally, \hessj{1804} is was suggested to be SNR G8.7$-$0.1
...
\hessj{1018}, \hessj{1507}, \hessj{1834}, and \verj{2016}.

\hessj{1018} is in the region of 
the $\gamma$-ray binary \onefglj{1018.6}
\citep{the-fermi-lat-collaboration_2012a_periodic-emission}
and also 
