...
the \fermi Gamma-ray Space Telescope in \secref{fermi_telescope}.
Next, we discuss the history of pulsars and \ac{PWN} in
\secref{pulsars_and_pwn} and the major source classes detected by the
\ac{LAT} in \secref{sources_detected_fermi}.  Finally, we present the
major radiation processes that occur in high-energy astrophysics in
\secref{radiation_processes}.

In \chapref{pulsar_pwn_system}, we discuss our current knowledge of
the pulsar/\ac{PWN} system.  We discuss the formation of a pulsar
in \secref{neutron_star_formation} and the time evolution of pulsars
in \secref{pulsar_evolution}.  Then, we describe the magnetosphere of
the pulsar in \secref{pulsar_magnetosphere} and the structure of the
\acp{PWN} in \secref{pwn_structure}.  Finally, we describe the energy
spectrum emitted from \acp{PWN} in \secref{pwn_emission}.
...
for $\gamma$-ray data in \secref{motivations_maximum_likelihood}
...
\ac{LAT} sources in \secref{validate_known}.  Next, we develop
a method to estimate systematic errors associated with studying
extended sources in \secref{systematic_errors_on_extension}.  Then,
we develop a method to search for new spatially-extended sources in
\secref{extended_source_search_method}.  Finally, we discuss the new
spatially-extended sources that we detect in \secref{new_ext_srcs_section}
and discuss the population of spatially-extended \ac{LAT} sources in
\secref{extended_source_discussion}.


In \chapref{offpeak}, we perform a search for new spatially-extended
sources in the off-peak regions of \ac{LAT}-detected pulsars.
First, we develop a new method to define the off-peak region in
\secref{peak_definition}.  Then, we describe the analysis method used to
search these regions for new \ac{PWN} in \secref{off_peak_analysis}
and the results of this search in \secref{off_peak_results}.
Finally, we discuss several sources detected with this method in
\secref{off_peak_individual_source_discussion}.

In \chapref{tevcat}, we perform a search for $\gamm$-ray emission
...
and age of their associated pulsars.  Finally, in \chapref{outlook}
